...
function SocialIcon({
  ...
  return (
    <a
      href={href}
      aria-label={label}
      className="flex h-11 w-11 items-center justify-center rounded-lg bg-white text-[#1E3A8A] shadow-md transition-all duration-300 hover:scale-105 hover:shadow-lg"
    >
      {children}
    </a>
  );
          ...
          <div className="px-4 py-12 lg:border-r lg:border-white/20 lg:px-8 lg:py-16">
            ...
            <form className="mt-6 flex max-w-md flex-col gap-3 sm:flex-row">
              ...
              <button
                type="submit"
                className={cn(
                  "rounded-md bg-white px-6 py-3 font-semibold text-[#1E3A8A] transition-colors hover:bg-white/90",
                  isAr && "font-[var(--font-noto-arabic)]"
                )}
              >
                {t.newsBtn}
